...
class Pipeline:
    ...
    @staticmethod
    def pipeline(img):
        line, M, Minv, cameraMat, distCoeffs = Pipeline.line, Pipeline.M, Pipeline.Minv, Pipeline.cameraMat, Pipeline.distCoeffs
        if (line is None or M is None or Minv is None or cameraMat is None or distCoeffs is None):
            raise NotImplementedError
            
        img_size = (img.shape[1], img.shape[0])
        width, height = img_size
        img = cameraCalibration.undistort(np.copy(img), cameraMat, distCoeffs)
